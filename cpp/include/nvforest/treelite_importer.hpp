 ...
 *
 * @tparam layout The in-memory layout for nodes to be loaded into nvForest
 */
...
struct treelite_importer {
  auto static constexpr const traversal_order = []() constexpr {
    if constexpr (layout == tree_layout::depth_first) {
      return nvforest::traversal::forest_order::depth_first;
    } else if constexpr (layout == tree_layout::breadth_first) {
      return nvforest::traversal::forest_order::breadth_first;
    } else if constexpr (layout == tree_layout::layered_children_together) {
      return nvforest::traversal::forest_order::layered_children_together;
    } else {
      static_assert(layout == tree_layout::depth_first,
                    "Layout not yet implemented in treelite importer for nvForest");
    }
  }();

  auto get_node_count(treelite::Model const& tl_model)
  {
    return nvforest::integration::tree_accumulate(
  ...
  auto get_offsets(treelite::Model const& tl_model)
  {
    ...
    nvforest::integration::node_transform<traversal_order>(
      tl_model,
  ...
  {
    return nvforest::integration::node_accumulate<traversal_order>(
      tl_model,
      index_type{},
    ...
    if constexpr (variant_index != std::variant_size_v<decision_forest_variant>) {
      if (variant_index == target_variant_index) {
        ...
        if constexpr (traversal_order ==
                      nvforest::traversal::forest_order::layered_children_together) {
        ...
        nvforest::integration::node_for_each<traversal_order>(
          tl_model,
   ...
   *
   * Load a model from Treelite to a nvForest forest_model. The model will be
 ...
 * object) to a nvForest forest_model. The model will be inspected to determine the
