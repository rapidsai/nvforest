...
std::unique_ptr<treelite::Model> convert_degenerate_trees(treelite::Model const& tl_model)
{
  bool contains_degenerate =
    cuforest::integration::tree_accumulate(tl_model, false, [](auto&& contains, auto&& tree) {
      return contains || tree.IsLeaf(cuforest::integration::TREELITE_NODE_ID_T{});
    });
  ...
  if (contains_degenerate) {
    ...
    std::visit(
      [](auto&& concrete_tl_model) {
        ...
        const auto root_id  = cuforest::integration::TREELITE_NODE_ID_T{};
