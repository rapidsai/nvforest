...
#define CUFOREST_SPEC(variant_index) \
  std::variant_alternative_t<variant_index, cuforest::detail::specialization_variant>

/* Macro which expands to a full declaration of a forest type corresponding to
 * the given variant index. */
#define CUFOREST_FOREST(variant_index)                          \
  forest<CUFOREST_SPEC(variant_index)::layout,                  \
         typename CUFOREST_SPEC(variant_index)::threshold_type, \
         typename CUFOREST_SPEC(variant_index)::index_type,     \
         typename CUFOREST_SPEC(variant_index)::metadata_type,  \
         typename CUFOREST_SPEC(variant_index)::offset_type>
