# cuForest Implementation
This document is intended to provide additional detail about this
implementation of cuForest to help guide future cuForest contributors.
Since cuForest attempts to provide a unified CPU/GPU codebase that
...
**A NOTE ON THE `raft_proto` NAMESPACE:** In addition to cuForest-specific code, the new
...
This cuForest implementation now makes use of a build-time variable
`CUFOREST_ENABLE_GPU` to determine whether or not to compile CUDA code. If
`CUFOREST_ENABLE_GPU` is not set, cuForest is guaranteed to compile without nvcc, access
...
indicating whether or not `CUFOREST_ENABLE_GPU` is set, which we introduce as
...
if `CUFOREST_ENABLE_GPU` is not defined.
...
* `constants.hpp`: Contains constant values that may be useful in working
  with cuForest in other C++ applications
...
* `exceptions.hpp`: Provides definitions for all custom exceptions that
  might be thrown within cuForest and need to be handled by an external
  application.
...
* `decision_forest_builder.hpp`: Provides generic tools for building
  cuForest forests from some other source. In the current cuForest codebase, the
...
* `integration/`: This directory contains tools for integrating with forest
  model sources. Currently, it contains glude code for integrating with Treelite.
* `traversal/`: This directory contains tools for traversing nodes of a forest
  in a variety of ways. These tools are not intended for high-speed traversal but
  for systematically extracting information from existing forest-like data structures
  in order to facilitate high-speed forest operations.

### Source Files
The cuForest source files contain no implementation details. They
merely use the macros defined in
`include/cuforest/detail/specializations` to indicate the template
