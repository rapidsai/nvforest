...
#include <cuforest/decision_forest.hpp>
...
struct treelite_importer {
  auto static constexpr const traversal_order = []() constexpr {
    if constexpr (layout == tree_layout::depth_first) {
      return cuforest::traversal::forest_order::depth_first;
    } else if constexpr (layout == tree_layout::breadth_first) {
      return cuforest::traversal::forest_order::breadth_first;
  ...
  {
    return cuforest::integration::tree_accumulate(
  ...
  auto get_offsets(treelite::Model const& tl_model)
  {
    ...
    cuforest::integration::node_transform<traversal_order>(
      tl_model,
  ...
  {
    return cuforest::integration::node_accumulate<traversal_order>(
      tl_model,
      index_type{},
                                  ...
                                  raft_proto::cuda_stream stream   = raft_proto::cuda_stream{})
  {
    ...
    if constexpr (variant_index != std::variant_size_v<decision_forest_variant>) {
      if (variant_index == target_variant_index) {
        ...
        if constexpr (traversal_order == cuforest::traversal::forest_order::layered_children_together) {
        ...
        cuforest::integration::node_for_each<traversal_order>(
          tl_model,
