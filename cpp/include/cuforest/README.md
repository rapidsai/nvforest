# cuForest Inference Library
RAPIDS cuForest Inference Library provides accelerated inference for
tree-based machine learning models. Unlike packages like XGBoost,
LightGBM, or even Scikit-Learn/cuML's random forest implementations, cuForest
...
This directory contains an implementation of cuForest which
...
offering details on both how to use cuForest as a library and how to work with it
as a cuForest contributor.

## C++ Usage
All headers required to make use of cuForest in another C++ project are
available in the top-level include directory. The `detail` directory
contains implementation details that are not required to use cuForest and which
will certainly change over time.

**A NOTE ON THE `raft_proto` NAMESPACE:** For the first iteration of this cuForest
...
### Importing a model
cuForest uses Treelite as a common translation layer for all its input types.
...
We then import the Treelite model into cuForest via the
...
auto cuforest_model = import_from_treelite_model(
...
CUDA headers if you are working with a CPU-only build of cuForest. This
...
auto num_outputs = cuforest_model.num_outputs();  // Outputs per row
...
iteration of cuForest's forest evaluation algorithm. On GPU, any power of 2 from 1 to 32
...
## Learning More
While the above usage summary should be enough to get started using cuForest in
